...
class Editor(QsciScintilla):
    ...
    def __init__(self, main_window, parent=None, path: Path = None,  python_file=True, env=None):
        ...
        # don't use tabs, otherwise jedi can't work
        ...
        self.SendScintilla(self.SCI_STYLESETBACK,
                           self.STYLE_DEFAULT, QColor("#282c34"))
        ...
        # debug_circle = QImage("./src/icons/red_dot.png").scaled(QSize(13, 13))
        # # smooth circle
        ...
        self.indicatorDefine(
            QsciScintilla.IndicatorStyle.SquigglePixmapIndicator, 0)
    ...
    def current_file_changed(self, value: bool):
        ...
        if value:
            ...
            if self.main_window.tab_view.tabText(curr_indx).startswith("*"):
                ...
                self.main_window.setWindowTitle(
                    self.main_window.windowTitle()[1:])
    ...
    def keyPressEvent(self, e: QKeyEvent) -> None:
        if e.modifiers() == Qt.KeyboardModifier.ControlModifier and e.key() == Qt.Key.Key_Space:
            if self.is_python_file:
                ...
                self.auto_completer.get_completion(
        ...
        # UPDATED EP 9
        if e.modifiers() == Qt.KeyboardModifier.ControlModifier and e.text() == "/":  # COMMENT SHORTCUT
            if self.hasSelectedText():
                start, startl, end, endl = self.getSelection()
                self.setSelection(start, 0, end, self.lineLength(end)-1)
                self.replaceSelectedText(
                    self.toggle_comment(self.selectedText()))
